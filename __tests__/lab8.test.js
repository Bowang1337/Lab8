describe('Basic user flow for SPA ', () => {
  ...
  it('Test5: On first Entry page - checking <entry-page> contents', async () => {
    /*
      ...
      */
      ...
      expect(result.title).toEqual('You like jazz?');
      expect(result.date).toEqual('4/25/2021');
      expect(result.content).toEqual("According to all known laws of aviation, there is no way a bee should be able to fly. Its wings are too small to get its fat little body off the ground. The bee, of course, flies anyway because bees don't care what humans think is impossible.");
      expect(result.image['src']).toEqual('https://i1.wp.com/www.thepopcornmuncher.com/wp-content/uploads/2016/11/bee-movie.jpg?resize=800%2C455');
      expect(result.image['alt']).toEqual('bee with sunglasses');
  }, 10000);
  ...
  it('Test11: Clicking the back button once should bring the user back to http://127.0.0.1:5501/', async() => {
    // implement test11: Clicking on the back button should update the URL’
    await page.goBack();
    expect(page.url()).toEqual('http://127.0.0.1:5501/');
  });
  ...
  it('Test12: When the user is on the homepage, the header title should be “Journal Entries"', async () => {
    ...
    await expect(result).toEqual('Journal Entries');
  });
  ...
  // define and implement test17: On the home page the <body> element should not be editable
  it('Test17: On the home page the <body> element should not be editable', async () => {
    await page.goBack();
    const result = await page.evaluate(() => {
  ...
  it('Test20: On third Entry page - checking <entry-page> contents', async () => {
    /*
     implement test20: Clicking on the thrid journal entry should contain the following contents: 
        { 
